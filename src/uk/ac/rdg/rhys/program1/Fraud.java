...
public class Fraud {
	...
	public Fraud(VBox mainScreen) {
		...
		crime = new Crime(CrimeType.FRAUD, caseNumber);
	...
	public void mainMenu(BorderPane border, StackPane stack, Button back) {
		...
		VBox newFraudScreen = new VBox(10);
		GridPane grid = new GridPane();

		// Align everything centrally
		newFraudScreen.setAlignment(Pos.CENTER);
		...
		newFraudScreen.getChildren().addAll(titleLabel, grid);

		// Remove previous screen from stack and add the current one
		stack.getChildren().remove(0);
		stack.getChildren().add(newFraudScreen);
	...
	private void addAccused(BorderPane border, StackPane stack, Button back) {
		...
		add.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				// Temporary variable to represent the current objects
	...
	private void specific(BorderPane border, StackPane stack, Button back) {
		...
		TextField type = new TextField(crime.getSpecific().getFraudType());
		ComboBox<FraudTarget> target = new ComboBox<>();
		target.getItems().setAll(FraudTarget.values());
		target.setValue(crime.getSpecific().getFraudTarget());
		TextField cost = new TextField("" + crime.getSpecific().getFraudCost());
		...
		cost.addEventFilter(KeyEvent.KEY_TYPED, restrictInput);

		// Disable add button if field is empty
		add.disableProperty().bind(
				Bindings.isEmpty(type.textProperty()).or(
						Bindings.isNull(target.valueProperty()).or(
								Bindings.isEmpty(summary.textProperty()))));

		// Add form elements to grid
		grid.add(new Label("Case Summary: *"), 0, 0);
		grid.add(summary, 1, 0);
		grid.add(new Label("Fraud Type: *"), 0, 1);
		grid.add(type, 1, 1);
		grid.add(new Label("Fraud Target: *"), 0, 2);
		grid.add(target, 1, 2);
		grid.add(new Label("Fraud Damage Cost:"), 0, 3);
		grid.add(cost, 1, 3);
		...
		add.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				localSpecific.setFraudType(type.getText());
				localSpecific.setFraudTarget(target.getValue());

				// Prevent an error if the cost field is empty
				try {
					localSpecific.setFraudCost(Integer.parseInt(cost.getText()));
				} catch (Exception ex) {
					localSpecific.setFraudCost(0);
				}
