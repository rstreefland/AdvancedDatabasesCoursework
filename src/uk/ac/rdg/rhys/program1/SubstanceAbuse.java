...
public class SubstanceAbuse {
	...
	public SubstanceAbuse(VBox mainScreen) {
		...
		crime = new Crime(CrimeType.SUBSTANCEABUSE, caseNumber);
	...
	public void mainMenu(BorderPane border, StackPane stack, Button back) {
		...
		VBox newSubstanceAbuseScreen = new VBox(10);
		GridPane grid = new GridPane();

		// Align everything centrally
		newSubstanceAbuseScreen.setAlignment(Pos.CENTER);
		...
		accusedButton.prefHeightProperty().bind(
				border.heightProperty().divide(5));
		...
		grid.add(witnessesButton, 1, 5);
		grid.add(evidenceButton, 0, 6);
		grid.add(specificButton, 1, 6);

		// Add title and grid to the screen
		newSubstanceAbuseScreen.getChildren().addAll(titleLabel, grid);

		// Remove previous screen from stack and add the current one
		stack.getChildren().remove(0);
		stack.getChildren().add(newSubstanceAbuseScreen);
	...
	private void addAccused(BorderPane border, StackPane stack, Button back) {
		...
		Label convictionsLabel = new Label("Previous Conviction");
	...
	private void addWitness(BorderPane border, StackPane stack, Button back) {
		...
		add.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				// Copy localWitness object into the ArrayList
	...
	private void specific(BorderPane border, StackPane stack, Button back) {
		...
		TextField substanceName = new TextField(crime.getSpecific()
				.getSubstanceName());

		ComboBox<SubstanceType> substanceType = new ComboBox<>();
		substanceType.getItems().setAll(SubstanceType.values());
		substanceType.setValue(crime.getSpecific().getSubstanceType());

		ComboBox<SubstanceClassification> substanceClassification = new ComboBox<>();
		substanceClassification.getItems().setAll(
				SubstanceClassification.values());
		substanceClassification.setValue(crime.getSpecific()
				.getSubstanceClassification());

		ComboBox<SubstanceConsumption> substanceConsumption = new ComboBox<>();
		substanceConsumption.getItems().setAll(SubstanceConsumption.values());
		substanceConsumption.setValue(crime.getSpecific()
				.getSubstanceConsumption());

		TextField quantitySeized = new TextField(crime.getSpecific()
				.getQuantitySeized());

		TextField streetValue = new TextField(Integer.toString(crime
				.getSpecific().getStreetValue()));

		ComboBox<SubstanceIntentToSupply> substanceIntentToSupply = new ComboBox<>();
		substanceIntentToSupply.getItems().setAll(
				SubstanceIntentToSupply.values());
		substanceIntentToSupply.setValue(crime.getSpecific()
				.getSubstanceIntentToSupply());

		TextArea associatedCrimes = new TextArea(crime.getSpecific()
				.getAssociatedCrimes());

		summary.setMaxSize(300, 100);
		associatedCrimes.setMaxSize(300, 100);

		// Create form label and set style
		Label specificLabel = new Label("Crime Specific Details");
		specificLabel.getStyleClass().add("label-header");

		streetValue.addEventFilter(KeyEvent.KEY_TYPED, restrictInput);

		// Disable add button if field is empty
		add.disableProperty().bind(
				Bindings.isEmpty(substanceName.textProperty())
						.or(Bindings.isEmpty(summary.textProperty()))
						.or(Bindings.isNull(substanceType.valueProperty()))
						.or(Bindings.isNull(substanceClassification
								.valueProperty())));

		// Add form elements to grid
		grid.add(new Label("Case Summary: *"), 0, 0);
		grid.add(summary, 1, 0);
		grid.add(new Label("Substance name: *"), 0, 1);
		grid.add(substanceName, 1, 1);
		grid.add(new Label("Substance type: *"), 0, 2);
		grid.add(substanceType, 1, 2);
		grid.add(new Label("Substance classification: *"), 0, 3);
		grid.add(substanceClassification, 1, 3);
		grid.add(new Label("Substance consumption method:"), 0, 4);
		grid.add(substanceConsumption, 1, 4);
		grid.add(new Label("Quantity seized:"), 0, 5);
		grid.add(quantitySeized, 1, 5);
		grid.add(new Label("Street value:"), 0, 6);
		grid.add(streetValue, 1, 6);
		grid.add(new Label("Intent to supply:"), 0, 7);
		grid.add(substanceIntentToSupply, 1, 7);
		grid.add(new Label("Associated crimes:"), 0, 8);
		grid.add(associatedCrimes, 1, 8);
		...
		add.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				localSpecific.setSubstanceName(substanceName.getText());
				localSpecific.setSubstanceType(substanceType.getValue());
				localSpecific
						.setSubstanceClassification(substanceClassification
								.getValue());
				localSpecific.setSubstanceConsumption(substanceConsumption
						.getValue());
				localSpecific
						.setSubstanceIntentToSupply(substanceIntentToSupply
								.getValue());
				localSpecific.setQuantitySeized(quantitySeized.getText());
				localSpecific.setAssociatedCrimes(associatedCrimes.getText());

				// Prevent an error if the cost field is empty
				try {
					localSpecific.setStreetValue(Integer.parseInt(streetValue
							.getText()));
				} catch (Exception ex) {
					localSpecific.setStreetValue(0);
				}
